...
// declare that this client is a mesh peer update watcher.
...
func (c *Client) recvTimeout(timeout time.Duration) (m ReceivedMessage, err error) {
	...
	for {
		...
		case frameServerInfo:
			...
			if c.isWatcher && si.AtomicWatchConn != true {
				return nil, errors.New("incompatible mesh peer does not support new watch connections format; please upgrade")
			}
