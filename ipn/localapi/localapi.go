...
func (h *Handler) serveDebugWebClient(w http.ResponseWriter, r *http.Request) {
	...
	if id := r.URL.Query().Get("id"); id != "" {
		noiseURL = fmt.Sprintf("https://unused/machine/webclient/wait/%s/to/%s/%s", data.Src, dst, id)
	} else {
		noiseURL = fmt.Sprintf("https://unused/machine/webclient/action/%s/to/%s", data.Src, dst)
	}
